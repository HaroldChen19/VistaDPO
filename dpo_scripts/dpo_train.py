        ...
        try:
            ...
            if self.training_modal == 'image':
                ...
                def parse_bbox(bbox_str):
                    json_str = bbox_str.replace("'", '"')
                    bbox_dict = json.loads(f"{{{json_str}}}")
                    return bbox_dict

                video_file = data_dict['video']
                video_folder = self.data_args.video_folder
                processor = self.data_args.video_processor
                processor_img = self.data_args.image_processor
                video_path = os.path.join(video_folder, video_file)
                # print(video)
                # import pdb; pdb.set_trace()
                # video = processor(video_path, return_tensors='pt')['pixel_values'][0] # torch.Size([3, 8, 224, 224])

                if 'bbox' in data_dict:
                    bbox = data_dict['bbox']
                    bbox_dict = parse_bbox(bbox)

                    clip_file = data_dict['clip_w']
                    clip_path = os.path.join(video_folder, clip_file)

                    clip_l_file = data_dict['clip_l']
                    clip_l_path = os.path.join(video_folder, clip_l_file)

                    frame_file = data_dict['frame']
                    frame_file_path = os.path.join(video_folder, frame_file)
                    # import pdb; pdb.set_trace()
                    image_files = [f for f in os.listdir(frame_file_path) if f.endswith('.jpg')]

                    image_path = os.path.join(frame_file_path, image_files[0])
                    # import pdb; pdb.set_trace()
                    frame_path = Image.open(image_path).convert('RGB')

                # frame_path = Image.open(os.path.join(video_folder, frame_file)).convert('RGB')

                # import pdb; pdb.set_trace()

                #### video = video_w
                # video, video_l = processor(video_path, bbox=bbox_dict, return_tensors='pt')
                video = processor(video_path, return_tensors='pt')['pixel_values'][0]

                N = video.shape[0]


                # video_l = video[shifted_indices]
                # video_l = video.clone
                video_l = torch.rand_like(video)

                if 'bbox' in data_dict:
                    clip = processor(clip_path, return_tensors='pt')['pixel_values'][0]

                    clip_l = processor(clip_l_path, return_tensors='pt')['pixel_values'][0]

                    frame, frame_l = processor_img(frame_path, bbox=bbox_dict, return_tensors='pt')
                ...
                data_dict['prompt'] = prompt
                has_X = 'video'
                # data_dict['video_w'] = video_w
                data_dict['video_l'] = video_l
                if 'bbox' in data_dict:
                    data_dict['clip'] = clip
                    data_dict['clip_l'] = clip_l
                    data_dict['frame'] = frame
                    data_dict['frame_l'] = frame_l
            else:
...
class DPODataCollator(DPODataCollatorWithPadding):
    ...
    def __call__(self, features: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        Xs, Xs_l, keys, Xclip, Xclip_l, Xframe, Xframe_l = [], [], [], [], [], [], []
        for feature in features:
            ...
            batch_element = self.tokenize_batch_element(prompt, chosen, rejected, rejected_ir, has_X=has_X)
            tokenized_batch.append(batch_element)

        # return collated batch
        padded_batch =  self.collate(tokenized_batch)
        padded_batch['images'] = [Xs, keys]  # we do not change the key's name.
        padded_batch['images_l'] = [Xs_l, keys]
        if Xclip != []:
